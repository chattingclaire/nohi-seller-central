...
export default function UGCPage() {
  ...
  const translations = {
    ...
    subtitle: language === "zh" ? "管理用户生成的内容。" : "Manage user-generated content.",
    automateNote: "Automate with Nohi PRO",
  ...
  const handleAddUGC = (item: Omit<UGCAsset, "id" | "date">) => {
    setUGC(prev => [{
      ...item,
      id: String(Date.now()),
  ...
  return (
    <SectionWrapper
      ...
      subtitle={<>{translations.subtitle} <span className="text-xs text-primary font-medium">{translations.automateNote}</span></>}
    >
      {/* Back Link */}
      <Link 
      ...
      {/* Stats */}
      <div className="rounded-xl bg-secondary/50 p-4 mb-4 inline-block">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        {filteredUGC.map((item) => (
          <div
            key={item.id}
            className="rounded-2xl overflow-hidden flex flex-col group bg-secondary/50"
          >
            {/* Media Preview */}
            <div className="h-32 bg-secondary flex items-center justify-center relative">
              ...
              <div className="absolute top-2 left-2 flex items-center gap-1.5">
                <Badge className={`text-xs ${platformColors[item.platform]}`}>
                  {item.platform}
                </Badge>
...
function AddUGCDialog({
  ...
  onAdd: (item: Omit<UGCAsset, "id" | "date">) => void
  language: string
}) {
